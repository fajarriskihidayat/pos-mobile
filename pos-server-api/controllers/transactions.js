...
exports.detailTransaction = async (no_order) => {
  const query = await db.query(
    "SELECT * FROM transactions AS t INNER JOIN  transaction_detail AS i ON t.no_order = i.no_order LEFT JOIN products AS p ON i.id_product = p.id WHERE t.no_order = ?",
    no_order
  );
  ...
  if (!query.error) {
    ...
    for (let i in query) {
      listDetail.push({
        ...
        product: query[i].name,
        quantity: query[i].quantity,
        img_product: query[i].img_product,
      });
    }

    listTransactions = {
      no_order: query[0].no_order,
      total_price: query[0].total_price,
      paid_amount: query[0].paid_amount,
      products: listDetail,
    };
